...
void mexFunction(int nlhs, mxArray *plhs[] , int nrhs, const mxArray *prhs[])
{
    ...
    unsigned int i;
    unsigned int t_in_days = 1u;

    /* Input arguments */
    double x,    y,    z,      /* initial positions */
           xdot, ydot, zdot;   /* initial velocities */

    double *dts,               /* time steps */
           *statevec;          /* state vectors */

    double muC;                /* std. grav. parameter of central body */
    char *tunits = "days";     /* time units */
    unsigned int numdts;       /* number of time steps, loop indicess */
    ...
    double nu0, r1m, beta, P, DeltaUFactor;
    ...
    if (beta > 0) {
        P = TWO_PI * muC * pow(beta, -3.0/2.0);
        DeltaUFactor = TWO_PI * pow(beta, -5.0/2.0);
    }
    ...
    for(i = 0u; i < numdts; ++i)
    {
        ...
        /* period effects */
        if (beta > 0)
            DeltaU = DeltaUFactor * floor((dt + P/2.0 - 2.0*nu0/beta)/P);

        /* loop until convergence of the time step */
        deltaT = t - dt;
        while (fabs(deltaT) > 1e-3) /* tolerance in seconds */
        {
            double bu, q;

            double A = 1.0, cFrac     = 1.0,
                   B = 1.0, cFracPrev = 2.0;
            ...
            while (fabs(cFrac-cFracPrev) > 1e-16)
            {
                k = -k;                 l += 2;
                d += 4*l;               n += (1+k)*l;
                A = d/(d - n*A*q);      B *= (A - 1.0);
